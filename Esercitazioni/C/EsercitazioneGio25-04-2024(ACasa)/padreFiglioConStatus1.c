...
int main()	/* nessun parametro necessario */
{
   ...
   int ritorno;    /* per filtrare valore di uscita del figlio  */
	...
	else
	{
        ...
        /* di nuovo come nell'esercizio padreSenzaFigli.c il codice potrebbe essere, 'a monte', moto piu' complicato e potrebbe essere NON chiaro il fatto che e' stato creato un solo processo figlio : quindi bisogna sempre controllare il valore di ritorno della wait! */
	if (wait((int *)0) < 0) /* in questo caso si e' usata la wait disinteressandoci del valore di ritorno dato che avremo errore! */
       	{
