...
int main ()
{
	...
	/* padre (!?!) */
	if (wait((int *)0) < 0)	/* nonostante questo processo non abbia creato figli, invoca la primitiva wait; chiaramente il codice potrebbe essere, 'a monte', molto piu'¹ complicato e potrebbe essere NON chiaro il fatto che non e' stato creato alcun processo figlio: quindi bisogna sempre controllare il valore di ritorno della wait! */
	{
