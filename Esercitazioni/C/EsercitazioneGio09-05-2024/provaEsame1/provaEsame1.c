...
int main(int argc, char *argv[])
{
   ...
   int N; 		/* N e' il numero di file e quindi di processi da creare (uno per ogni file passato) */
   ...
   int i;		/* indice processi */
   int tot=0; 		/* tot variabile usata dal padre per contare i caratteri scritti su standard output: si suppone che un int sia sufficiente! */
   int nr0, nr1; 	/* nr0 numero di caratteri letti dalla pipe 'pari' e nr1  numero di caratteri letti dalla pipe 'dispari' */
   char ch, ch0, ch1; 	/* ch e' il carattere usato da ogni figlio per leggere i singolo caratteri da ogni file; ch0 e' il carattere letto dal padre sulla pipe 'pari' e ch1 e' il carattere letto dal padre su quella 'dispari' */
   int pidFiglio, status, ritorno;    /* variabili per wait */
  	...
  	if (pipe(p[0]) < 0) 	/* pipe per i caratteri numerici che sara' usata in scrittura dai figli (dispari) che leggono dai file pari (F2, F4, ...) */
  	{ 	printf("Errore nella creazione della prima pipe\n");
    		exit(2);
  	}
  	if (pipe(p[1]) < 0) 	/* pipe per i caratteri alfabetici che sara' usata in scrittura dai figli (pari) che leggono dai file dispari (F1, F3, ...) */
  	...
  	for (i=0; i < N; i++)
  	{
			...
			*/
			...
			/* N.B. i processi di indice pari (ad esempio 0) leggono i file di indice dispari (ad esempio argv[1]) e quindi devono selezionare i caratteri alfabetici, mentre i processi di indice dispari (ad esempio 1) leggono i file di indice pari (ad esempio argv[2]) e quindi devono selezionare i caratteri numerici */ 
      			...
      			{
        			if ((((i % 2) == 0) && isalpha(ch)) || 		/* il file dispari e' letto in realta' da un processo di indice pari e quindi si controlla che il carattere letto sia un alfabetico */
            				(((i % 2) == 1) && isdigit(ch)))   	/* il file pari e' letto in realta' da un processo di indice dispari e quindi si controlla che il carattere letto sia un numerico */
          					write(p[(i+1) % 2][1], &ch, 1); /* comunica il carattere al padre, scrivendo sulla pipe 0 i numerici e sulla pipe 1 gli alfabetici */
      			}
      			exit(ch); /* e' specificato di tornare l'ultimo carattere letto dal figlio */
	...
	/* chiude i lati di scrittura di entrambe le pipe*/
  	...
  	while ((nr0 != 0) || (nr1 != 0))
  	{
		...
		/* OSSERVAZIONE: si e' deciso di usare una write invece che un printf per poter sfruttare questo discorso del valore eventuale a 0 di nr0 o nr1 */
