...
int main  (int argc, char **argv) 
{
   ...
   char *com1[10], *com2[10];		/* array di 10 puntatori a char, supposti sufficienti per memorizzare il primo comando (con i suoi parametri) e il secondo comando (con i suoi parametri) che sono presenti nel piping di comandi */ 
   int pid;                   		/* pid per fork */
   int piped[2];                   	/* piped per pipe */
   int pidFiglio, status, ritorno;      /* per wait padre */

   	/* questo processo e' quello che simula la SHELL: lo chiamiamo PROCESSO P1 */
	...
	{ 	printf("Numero dei parametri errato %d: ci vogliono almeno tre parametri\n", argc); 
		exit(1); 
	}
	...
	/* N.B. NON andiamo a COPIARE le stringhe corrispondenti ai vari argv[i] (anche perche' NON abbiamo allocato la memoria per i vari puntatori di com1 e com2, ma semplicemente copiamo il puntatore ai vari argv[i] nei vari elementi di com1 e com2 che poi verranno usati nelle exec */
	for (i=1; i < argc && strcmp (argv[i], "!"); i++) 
		com1[i-1] = argv[i];	/* attenzione all'indice! */
    	com1[i-1] = (char *)0;  /* terminatore della lista di stringhe */   
	i++;   
	for (j = 1; i < argc ; i++, j++) 
		com2[j-1] = argv[i];	/* di nuovo attenzione all'indice! */
  	com2[j-1] = (char *)0;  /* terminatore della lista di stringhe */

	/* creazione del figlio per eseguire il comando rappresentato dal piping */
	if ((pid = fork()) < 0)
	...
	/* padre PROCESSO P1:  attende il figlio P2 */
